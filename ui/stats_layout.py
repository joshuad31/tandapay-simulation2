...
class Stats_Layout:
    def __init__(self, other_vars, widget_width=None):
        ...
        self.ov4_label = "Null condition"
        self.ov4_tooltip = "Null hypothesis condition"

        self.ov5_label = "Alt condition"
        self.ov5_tooltip = "Alternative hypothesis condition"

        self.ov6_label = "Outcome to test"
        self.ov6_tooltip = "which outcome do you want to test?"

        self.ov7_label = "Value to test"
        self.ov7_tooltip = "which value do you want to test against? (should be a percentage. Accepts decimal values, such as 66.67)"
    ...
    def get_stats_vars(self) -> Other_Variables:
        ...
        self.ov.null_condition      = ConditionEnum(self.uief.getValue(self.ov4_label))
        self.ov.alt_condition       = ConditionEnum(self.uief.getValue(self.ov5_label))
        self.ov.test_outcome        = OutcomeEnum(self.uief.getValue(self.ov6_label))
        return self.ov
    ...
    def get_stats_layout(self) -> QVBoxLayout:
        ...
        options = [option.name for option in ConditionEnum]
        self.ov4_hbox = self.uief.make_dropdown_entry_element(self.ov4_label, options, self.ov4_tooltip, self.ov.null_condition.name)
        layout.addLayout(self.ov4_hbox)

        self.ov5_hbox = self.uief.make_dropdown_entry_element(self.ov5_label, options, self.ov5_tooltip, self.ov.alt_condition.name)
        layout.addLayout(self.ov5_hbox)
        
        options = [option.name for option in OutcomeEnum]
        self.ov6_hbox = self.uief.make_dropdown_entry_element(self.ov6_label, options, self.ov6_tooltip, self.ov.test_outcome.name)
        layout.addLayout(self.ov6_hbox)

        self.ov7_hbox = self.uief.make_float_entry_element(self.ov7_label, self.ov7_tooltip, self.ov.value_to_test, 0.0, 100.0)
